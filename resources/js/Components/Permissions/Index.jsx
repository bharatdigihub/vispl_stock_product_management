...
const PermissionIndex = ({ permissions, routes }) => {
    ...
    return (
        <GlobalLayout> {/* Wrap with GlobalLayout */}
            ...
            <table className={`tw-table-auto tw-w-full tw-border ${theme.border.sidebarInner}`}>
                <thead className="tw-bg-gray-900 tw-text-white">
                    <tr>
                        ...
                        <th className={`tw-border ${theme.border.sidebarInner} tw-p-3 tw-text-left tw-font-medium tw-max-w-20`}>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {permissions.map((permission) => (
                        <tr key={permission.id} className={`tw-border-b ${theme.border.sidebarInner}`}>
                            <td className={`tw-border ${theme.border.sidebarInner} tw-p-3`}>{permission.name}</td>
                            <td className={`tw-border ${theme.border.sidebarInner} tw-p-3 tw-max-w-20`}>
                                <PrimaryButton
                                    as="Link"
