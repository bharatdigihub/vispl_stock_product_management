...
const ColorIndex = () => {
    ...
    return (
        <GlobalLayout> {/* Wrap with GlobalLayout */}
            <h1 className="tw-text-2xl tw-font-bold tw-mb-6">Color List</h1>
            <PrimaryButton
            ...
            <div className="tw-overflow-x-auto">
                <table cellPadding={0} cellSpacing={0} className={`tw-table-fixed tw-w-full tw-border ${theme.border.sidebarOuter} tw-rounded-lg tw-shadow-sm tw-mt-4`}>
                    ...
                    <tbody>
                        {colors.map((color) => (
                            <tr key={color.id} className={`tw-border-b ${theme.border.sidebarOuter}`}>
                                <td className={`tw-border tw-w-48 ${theme.border.sidebarOuter} tw-p-3 ${theme.mode === "dark" ? 'tw-bg-gray-900 tw-text-gray-100' : 'tw-bg-gray-100'}`}>{color.name}</td>
